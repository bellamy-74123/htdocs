...
(function initChatbotWidget() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // 2. الاتصال عبر وسيط PHP إذا لزم الأمر
            if (!resData) {
                const candidatePaths = [
                    'http://localhost:8080/api/chatbot.php',
            ...
            if (!resData) {
                resData = queryWidgetDatabase(text, currentMeds);
            }
    ...
    function queryWidgetDatabase(query, medicines) {
        const q = (query || '').toLowerCase().trim();
        const meds = (medicines && medicines.length > 0) ? medicines : (typeof DEMO_MEDICINES !== 'undefined' ? DEMO_MEDICINES : []);

        if (q.includes('مرحبا') || q.includes('اهلا') || q.includes('السلام') || q.includes('هلا')) {
            return { reply: "أهلاً بك. المساعد متصل مباشرة بقاعدة بيانات الصيدلية للبحث عن الأسعار، توفر المخزون، والبدائل المتكافئة." };
        }
        ...
        // فحص الأعراض السريرية الشائعة
        const symptomRules = [
            { keywords: ['صداع', 'راس', 'رأس', 'شقيقة', 'شقيقه', 'مصدع'], title: 'الصداع والصداع النصفي', generics: ['باراسيتامول', 'إيبوبروفين', 'ديكلوفيناك'], advice: 'تناول قرص باراسيتامول أو بنادول إكسترا بعد الأكل مع شرب الماء.' },
            { keywords: ['حرارة', 'حراره', 'سخونة', 'سخونه', 'حمى', 'حمي', 'مكسر'], title: 'الحمى وارتفاع الحرارة', generics: ['باراسيتامول', 'إيبوبروفين'], advice: 'تناول الباراسيتامول كل 6-8 ساعات مع كمادات ماء فاتر وسوائل بكثرة.' },
            { keywords: ['حموضة', 'حموضه', 'حرقان', 'ارتجاع', 'قرحة', 'معدة'], title: 'حموضة وحرقة المعدة', generics: ['أوميبرازول', 'بانتوبرازول'], advice: 'تناول كبسولة أوميبرازول 20 مجم صباحاً قبل الإفطار بنصف ساعة.' },
            { keywords: ['كحة', 'كحه', 'سعال', 'بلغم', 'رشح', 'زكام', 'حلق'], title: 'السعال والرشح والتهاب الحلق', generics: ['باراسيتامول', 'أزيثرومايسين', 'أموكسيسيلين', 'سيتريزين'], advice: 'تناول الباراسيتامول لتسكين الحلق والحرارة ومضادات الهيستامين للرشح.' },
            { keywords: ['مفاصل', 'ركبة', 'ركبه', 'روماتيزم', 'عظام', 'ظهر', 'خشونة'], title: 'آلام المفاصل والفقرات', generics: ['سيليكوكسيب', 'ديكلوفيناك', 'ميلوكسيكام', 'إيبوبروفين'], advice: 'استخدام مضاد التهاب مثل سيلبركس أو فولتارين بعد الوجبات مباشرة.' },
            { keywords: ['اسنان', 'أسنان', 'ضرس', 'لثة', 'خراج'], title: 'ألم الأسنان والتهاب اللثة', generics: ['ديكلوفيناك', 'إيبوبروفين', 'أموكسيسيلين + كلافولانات'], advice: 'تناول مسكن سريع مثل كتافلام أو بروفين بعد الأكل مع زيارة طبيب الأسنان.' }
        ];
        ...
        for (const rule of symptomRules) {
            if (rule.keywords.some(k => q.includes(k))) {
                const matched = meds.filter(m => {
                    ...
                    return rule.generics.some(g => gen.includes(g) || nm.includes(g));
                }).sort((a, b) => (b.stock_quantity > 0 ? 0 : 1) - (a.stock_quantity > 0 ? 0 : 1) || (a.price - b.price));

                if (matched.length > 0) {
                    let reply = `### التقييم السريري: **${rule.title}**\n\n`;
                    reply += `#### الأدوية الموصى بها المتوفرة بالمخزن:\n`;
        ...
        return { reply: "لم أتمكن من العثور على الدواء المطلوب في قاعدة البيانات. يمكنك سؤالي عن أعراض المريض (مثل: 'دواء للصداع'، 'علاج لحموضة المعدة') أو كتابة اسم الدواء بوضوح." };
    }
